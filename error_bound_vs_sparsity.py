...
for thres in np.arange(0,1,0.1):
	...
	plt.axis('off')
	plt.savefig(path+'random_graph_thres_%s'%(thres)+'.png', dpi=200)
	plt.show()
...
for thres in np.arange(0,1,0.1):
	...
	lambda_2_list.extend([np.sort(evals)[1]])

plt.figure(figsize=(5,5))
plt.plot(np.arange(0,1,0.1), fitness_list, label='Fitness')
plt.plot(np.arange(0,1,0.1), smoothness_list, label='Smoothness')
plt.legend(loc=0, fontsize=12)
plt.ylabel('Fitness and Smoothness', fontsize=12)
plt.xlabel('Threshold', fontsize=12)
plt.tight_layout()
plt.savefig(path+'fitness_and_smoothness_threshold'+'.png', dpi=100)
plt.show()

plt.figure(figsize=(5,5))
plt.plot(np.arange(0,1,0.1), lambda_2_list)
plt.xlabel('Threshold', fontsize=12)
plt.ylabel('lambda_2', fontsize=12)
plt.savefig(path+'lambda_2_vs_sparsity'+'.png', dpi=100)
plt.show()

thres_list=[0,0.6,0.9]
emp_error_matrix=np.zeros((len(thres_list), item_num))
global_error_matrix=np.zeros((len(thres_list), item_num))
ind_error_matrix=np.zeros((len(thres_list), item_num))
...
plt.tight_layout()
plt.savefig(path+'emperical_error_sparsity'+'.png', dpi=100)
plt.show()
...
plt.ylabel('Error (MSE)', fontsize=12)
plt.tight_layout()
plt.savefig(path+'global_error_bound_sparsity'+'.png', dpi=100)
plt.show()
...
plt.ylabel('Error (MSE)', fontsize=12)
plt.tight_layout()
plt.savefig(path+'ind_error_bound_sparsity'+'.png', dpi=100)
plt.show()
...
for i in range(len(thres_list)):
	...
	plt.xlabel('sample size', fontsize=12)
	plt.ylabel('Error(MSE)', fontsize=12)
	plt.savefig(path+'error_bound_comparsion_vs_sparsity_thres_%s'%(thres_list[i])+'.png', dpi=100)
	plt.show()
